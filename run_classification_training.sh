#!/bin/bash
# =============================================================================
# run_classification_training.sh — Stage 2: Supervised Classification Training
#
# Fine-tunes a pre-trained SSL encoder head for particle classification.
# Requires a completed SSL experiment from Stage 1 (run_ssl_training.sh).
#
# Usage:
#   bash run_classification_training.sh                                  # default config
#   bash run_classification_training.sh /path/to/custom_config.yaml      # one config file
#   bash run_classification_training.sh /path/to/config_dir/             # BATCH: every *.yaml / *.yml in the dir
#
# Before running, edit the GPU CONFIGURATION block below and set the
# following fields in your config YAML:
#   model_initialization.pretraining.experiment_path  — path to SSL output dir (CHANGE ME)
#   data.train_data_path                               — labeled HDF5 file      (CHANGE ME)
#   logging.checkpoint_dir                      — output directory        (CHANGE ME)
# =============================================================================
...
cat <<'EOF'
 ...
 ╚═════╝ ╚═╝ ╚═════╝     ╚═╝  ╚═╝╚═╝╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝   ╚═╝
            ...
            Stage 2 — Supervised Classification Training

EOF
echo -e "${BOLD}${MAGENTA}════════════════════════════════════════════════════════════════════${NC}"
echo -e "${BOLD}${CYAN}   BioAirMet Classification Training - PTB, NPL                    ${NC}"
echo -e "${BOLD}${MAGENTA}════════════════════════════════════════════════════════════════════${NC}\n"
...
# GPU_MODE="multi"
# GPU_IDS="0,1,2,3"                 # CHANGE ME — comma-separated GPU indices
...
# --- Script config -----------------------------------------------------------
CONFIG_PATH="${1:-src/bioairmet/config/classification/config_general.yaml}"
...
[ -e "$CONFIG_PATH" ] || error "Path not found: $CONFIG_PATH
  Pass a config file, or a directory containing one or more configs:
    bash run_classification_training.sh /path/to/config.yaml
    bash run_classification_training.sh /path/to/config_dir/
  Default: src/bioairmet/config/classification/config_general.yaml"
...
echo -e "${BLUE}Mode               :${NC} classification"
...
echo "    model_initialization.pretraining.experiment_path  — Stage 1 output dir (CHANGE ME)"
echo "    data.train_data_path                               — labeled HDF5 file  (CHANGE ME)"
echo "    data.cat_map_path                           — category map .txt  (optional)"
echo "    logging.checkpoint_dir                      — output directory   (CHANGE ME)"
echo "    distributed.world_size                      — -1=all GPUs, 1=single"
echo "    distributed.gpu_ids                         — [] = all visible, [0]=single"
echo ""
...
run_one_config() {
    local cfg="$1"
    echo ""
    info "Starting classification training: ${cfg}"
    echo ""
    local rc=0
    bioairmet-train --config_path "${cfg}" --mode classification || rc=$?
    return "${rc}"
}
...
echo -e "${BOLD}${GREEN}✓ Classification training completed successfully!${NC}"
echo -e "${BOLD}${GREEN}════════════════════════════════════════════════════════════════════${NC}"
echo ""
echo -e "${CYAN}Outputs saved to:${NC}  logging.checkpoint_dir/<timestamp>_<project_name>/"
echo -e "  • best_classification_model.pth   — best validation metric checkpoint"
echo -e "  • last_classification_model.pth   — final epoch checkpoint"
echo -e "  • config.yaml          — exact config used for this run"
echo ""
echo -e "${CYAN}Next steps:${NC}"
echo "  Validate  :  bash run_validation_v2.sh"
echo "  Inference :  bash run_inference.sh"
echo "  Or directly:"
echo "    bioairmet-validate --experiment_dir <EXPERIMENT_DIR> --data_path <HDF5>"
echo "    bioairmet-inference --experiment_dir <EXPERIMENT_DIR> \\"
echo "                        --data_path <HDF5> --save_path ./inference_results"
echo ""
